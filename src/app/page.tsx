...
export default function Home() {
  ...
  return (
    <div className="relative flex flex-col w-full h-full bg-[#121A2A]" id="home">
      <Particles quantity={50} className="fixed w-full h-full"/>
      ...
      <div className="px-4 py-2 fixed w-full top-0 left-1/2 transform -translate-x-1/2 z-50 bg-[#121A2A] shadow-[0px_0.5px_20.16px_0px_rgba(0,_0,_0,_0.26)]">
        <div className={`container mx-auto flex justify-between items-center py-4 font-roboto`}>
          ...
          <button onClick={() => setMobileMenuOpen(prev => !prev)} className="z-50 flex flex-col justify-center items-center w-10 h-10 md:hidden relative group" aria-label="Toggle menu">
        ...
        <div className="relative flex justify-between w-full h-full">
          ...
          <AnimatePresence>
            {showProjectImage === 'xemtio' && (
              <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} transition={{ duration: 0.4 }} className="absolute top-1/2 right-4 transform -translate-y-1/2 hidden xl:block">
                <Image src={xemtio} width={650} height={650} alt="Xemtio project preview" className="rounded-lg w-[550px] 2xl:w-[650px]" />
              </motion.div>
            )}
          </AnimatePresence>
        </div>
        <div className="relative flex justify-between w-full h-full">
          ...
          <AnimatePresence>
            {showProjectImage === 'snet' && (
              <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} transition={{ duration: 0.4 }} className="absolute top-1/2 right-4 z-[100] transform -translate-y-1/2 hidden xl:block">
                <Image src={snet} alt="SNET project preview" className="rounded-lg w-[550px] 2xl:w-[650px]"/>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
        <div className="relative flex justify-between w-full h-full">
          <motion.div onClick={() => window.open('https://opgnedaca.vercel.app', '_blank')} onHoverStart={() => setShowProjectImage('opgnedaca')} onHoverEnd={() => setShowProjectImage(null)} className="group" initial={{ opacity: 0, y: 50 }} whileInView={{opacity: 1, y: 0}} transition={{ duration: 0.5, delay: 0.2 }}>
            <SpotlightCard className="flex flex-col text-left py-2 px-4 md:px-6 rounded-lg bg-transparent border-0 w-full max-w-[500px] md:w-[650px] md:max-w-full h-full z-50" spotlightColor="rgba(0, 229, 255, 0.2)">
              ...
              <h3 className="text-lg text-gray-500 pt-2">STILL IN PROGRESS</h3>
              <p className={`text-sm lg:text-lg font-extralight ${roboto.className} w-[90%] lg:w-full text-[#DEDEDE] mt-2 lg:mt-4`}>OPG Nedaća is a family-owned organic farm in Croatia, focused on growing eco-friendly fruit and producing high-quality natural products. The website is built with React and features a cart-based ordering system.</p>
            </SpotlightCard>
          </motion.div>
          ...
          <AnimatePresence>
            {showProjectImage === 'opgnedaca' && (
              <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} transition={{ duration: 0.4 }} className="absolute top-1/2 right-4 transform -translate-y-1/2 hidden xl:block">
                <Image src={opgnedaca} width={650} height={650} alt="OPG Nedaća project preview" className="rounded-lg w-[550px] 2xl:w-[650px]"/>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
      ...
      <div className="mt-36 py-2 flex flex-col justify-center items-center gap-4">
        ...
        <motion.h1 className={`text-[#DEDEDE] ${anton.className} text-4xl pb-8`} initial={{ opacity: 0, y: 50 }} whileInView={{opacity: 1, y: 0}} transition={{ duration: 0.5 }}>Marko Milković</motion.h1>
      </div>
    </div>
  );
}
